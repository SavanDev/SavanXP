...
int main(void) {
    ...
    rx_fd = savanxp_socket(SAVANXP_AF_INET, SAVANXP_SOCK_DGRAM, SAVANXP_IPPROTO_UDP);
    tx_fd = savanxp_socket(SAVANXP_AF_INET, SAVANXP_SOCK_DGRAM, SAVANXP_IPPROTO_UDP);
    if (rx_fd < 0 || tx_fd < 0) {
        eprintf("udptest: socket failed (%s)\n", result_error_string(rx_fd < 0 ? rx_fd : tx_fd));
        if (rx_fd >= 0) {
            savanxp_close((int)rx_fd);
        }
        if (tx_fd >= 0) {
            savanxp_close((int)tx_fd);
        }
        return 1;
    }
    ...
    address.port = 7000;
    status = savanxp_bind((int)rx_fd, &address);
    if (status < 0) {
    ...
    status = savanxp_sendto((int)tx_fd, message, strlen(message), &address);
    if (status < 0) {
    ...
    status = savanxp_recvfrom((int)rx_fd, buffer, sizeof(buffer) - 1, &remote, 1000);
    if (status < 0) {

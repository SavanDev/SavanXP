...
The art in `icons/16x16` and `icons/32x32` belongs to this repository: most of
it is generated locally with `tools/gen_desktop_source_art.py` and depends on no
external icons. `app-mediaplayer.png` is checked-in source art shared with the
optional FFmpeg port and is not regenerated by that script.

...
### What this build can decode, and who owns that answer

A program can now ask the system what it can play: `sx_media_codec_count` and
`sx_media_codec_at`, gathered from every registered provider and deduplicated.
There was **no such answer before** -- no table anywhere, not in the manifests and
not in the code -- so this is not a better list, it is the first one.

The interesting part is where the numbers come from. For a provider that ships a
whole codec library, the list is **generated by the build that linked it**:
`ports/ffmpeg/tools/gen_sxmedia_caps.py` reads FFmpeg's own `config_components.h`
for what was actually enabled and `libavcodec/codec_desc.c` for each name and
whether it is audio or video, and emits 24 decoders for the current configuration.
Nothing typed it, so it cannot drift from the build, and `--disable-decoder=h264`
makes the table stop claiming H.264 without anybody editing a C file.

The obvious implementation -- a host program that calls `av_codec_iterate` and
prints the table -- is the one that does not work, and the reason is worth
recording: the port's libav is a **cross-compiled static archive**, so a host
program cannot link it. Asking the library requires asking the target. Reading the
two files that configure generated is the same answer as text.

A table that is checked in would be a promise about a build nobody made, so the
generated file lands in the build directory and `ffmpeg_port_layout_test.py` fails
if one ever appears in the tree.

**What is deliberately not generated: the extensions.** An extension is a
container choice, which is policy, and `docs/SXE_FORMAT.md` already decides that
the system carries no compiled table of "which type is this extension" -- adding a
type is copying two PNGs and adding a line. A codec is not a preference: whether
H.264 decodes here is a fact about the build, and a program that guessed wrong
about it would show a file it then refuses to open. Those two tables deserve
different owners for the same reason, and generating the second is not a reason to
generate the first.
